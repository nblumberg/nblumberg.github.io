...
export function boardToQueryString(board: Cell[][]): string {
  let across: string[] = [];
  let down: string[] = [];
  let grid = '';
...
export function queryStringToBoard(): Cell[][] {
  const params = new URLSearchParams(window.location.search);
  ...
  let x = 1;
  let count = '';
  let cellNumbers: { ACROSS?: number; DOWN?: number; } | undefined;
...
export function queryStringToClues(): { across: string[], down: string[] } {
  const params = new URLSearchParams(window.location.search);
