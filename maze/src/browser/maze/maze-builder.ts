export type Cell = 'entrance' | 'exit' | 'door' | 'wall' | 'key' | 'sentinel' | 'nubbin' | undefined;
...
class MazeBuilder {
  ...
  partition(r1: number, r2: number, c1: number, c2: number): false | undefined {
  ...
  countSteps(array: number[][], r: number, c: number, val: number, stop: Cell): boolean | undefined {
  ...
  getKeyLocation(): [number, number] {

    let fromEntrance = this.initArray() as number[][];
    let fromExit = this.initArray() as number[][];
  ...
  display(id: string) {
    ...
    this.parentDiv = document.getElementById(id) as HTMLDivElement;
